...
let loadInstagram = async function(page, cnf) {
  ...
  await page.keyboard
    .type(process.env.USER_PROD)
    .catch(err => console.log('page.waitFor' + err));
  await page.click(cnf.selectors.password_field).catch(err => {
    console.log(' Error click on password_field  : ' + err);
  });

  await page.keyboard
    .type(process.env.PASS_PROD)
    .catch(err => console.log('page.keyboard.type password : ' + err));
  await page.click(cnf.selectors.login_button).catch(err => {
    console.log(' Error click on login button  : ' + err);
  });
  await page
    .waitForNavigation()
    .catch(err => console.log(' Error page.waitForNavigation : ' + err));
};
