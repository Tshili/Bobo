  ...
  for (let r = 1; r < 4; r++) {
    for (let c = 1; c < 4; c++) {
      ...
      if (url.includes('https://www.instagram.com/explore/locations')) {
        ...
        await page
          .click(
            '#react-root > section > main > article > div:nth-child(3) > div > div:nth-child(' +
              r +
              ') > div:nth-child(' +
              c +
              ') > a'
        ...
        await page
          .click(
            'div:nth-child(3) > div > div:nth-child(' +
              r +
              ') > div:nth-child(' +
              c +
              ') > a'
      ...
      if (hasEmptyHeart !== null && Math.random() < cnf.settings.like_ratio) {
        await page
          ...
          .catch(err => console.log('err when i want to like : ' + err));
...
let getAllFollowing = async function() {
  followings = await ops
    .getFollowing()
    .catch(err => console.log('getAllFollowing error : ' + err));
  return followings;
};
